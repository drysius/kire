...
import { activeKire, type SsgBuilder } from "./types";
...
export async function dev(opts: { port?: number } = {}) {
	...
	const server = createServer(async (req, res) => {
		...
		try {
			...
			if (devGeneratedFiles.has(cleanPath)) {
				...
				res.setHeader(
					"Content-Type",
					cleanPath.endsWith(".xml") ? "application/xml" : "text/plain",
				);
				res.end(content);
			...
			if (url.startsWith(prefixPath)) {
				...
				if (match) {
					...
					if (asset && asset.type === ext) {
						...
						Logger.warn(
							`[DEV] Asset 404. Hash: ${hash}. Cache has ${keys.length} items: ${keys.join(", ")}`,
						);
					}
				}
				res.statusCode = 404;
			...
			const candidates = [cleanUrl, `${cleanUrl}/index`];
			...
			if (html === null) {
				...
				const dynamicTemplates = allFiles.filter(
					(f) => f.includes("[") && f.endsWith(ext),
				);
				...
				for (const template of dynamicTemplates) {
					...
					if (paramName) {
						...
						if (urlMatch) {
							...
							const _kirePath = join(
								routesRelPath,
								relTemplatePath.slice(0, -ext.length),
							); // View expects path without ext? No, view expects relative path usually with ext or resolved.
	...
	server.listen(port, () => {
		Logger.success(
			`Server running at ${colors.brightWhite}http://localhost:${port}${colors.reset}`,
		);
	});
