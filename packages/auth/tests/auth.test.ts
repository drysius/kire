...
describe("KireAuth", () => {
	const createKire = (user: any = null) => {
		const kire = new Kire();
	...
	test("@auth and @else", async () => {
		...
		const htmlLogged = await kireLogged.render("@auth @user Logged in: {{ user.name }} @else Not Logged @end", { user: { name: "John", perms: [] } });
		expect(htmlLogged.trim()).toBe("Logged in: John");

		const kireGuest = createKire();
		const htmlGuest = await kireGuest.render("@auth @user Logged in: {{ user.name }} @else Not Logged @end", { user: null });
	...
	test("@guest and @notlogged", async () => {
		...
		const kireLogged = createKire();
		const htmlLogged = await kireLogged.render(template, { user: { name: "John" } });
	...
	test("@can and @notcan", async () => {
		...
		const htmlAdmin = await kireAdmin.render(template, { user: { name: "Admin", perms: ['admin'] } });
		...
		const htmlUser = await kireUser.render(template, { user: { name: "User", perms: ['user'] } });
	...
	test("@canany", async () => {
		...
		const htmlEditor = await kireEditor.render(template, { user: { perms: ['editor'] } });
		expect(htmlEditor.trim()).toBe("Staff");

		const kireUser = createKire();
		const htmlUser = await kireUser.render(template, { user: { perms: ['user'] } });
