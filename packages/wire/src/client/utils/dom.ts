...
export function findWireAttribute(
	target: HTMLElement,
	eventType: string,
	...
	while (el && el !== document.body) {
		...
		matchedAttribute = attrs.find(
			(a) => a === attributePrefix || a.startsWith(`${attributePrefix}.`),
		);
	...
	if (el) {
		...
		if (activeId) {
			...
			if (activeId.startsWith("wire:model=")) {
				newActive = el.querySelector(
					safeSelector("wire:model", activeId.substring(11)),
				);
			} else if (activeId.startsWith("wire:defer=")) {
				newActive = el.querySelector(
					safeSelector("wire:defer", activeId.substring(11)),
				);
