...
import { interceptAction, interceptMessage, interceptRequest } from "./core/interceptor";

//@ts-expect-error ignore
window.Alpine = Alpine;

function dispatch(name: string, params: any) {
    window.dispatchEvent(new CustomEvent(name, { detail: params, bubbles: true, composed: true, cancelable: true }));
}

function dispatchTo(componentName: string, name: string, params: any) {
    const components = getByName(componentName);
    components.forEach(component => {
        component.el.dispatchEvent(new CustomEvent(name, { detail: params, bubbles: false, composed: true, cancelable: true }));
    });
}
