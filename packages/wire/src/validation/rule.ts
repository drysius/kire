...
const SIMPLE_FORMATS: Array<{
	...
	},
	{
		name: "ipv6",
		check: (value) =>
			/^(([0-9A-Fa-f]{1,4}:){7}[0-9A-Fa-f]{1,4}|([0-9A-Fa-f]{1,4}:){1,7}:|::1|::)$/.test(
				value,
			),
	},
	{
...
export function buildRuleSchema(
	rules: string,
...
export function makeRuleDescriptor(
	ruleStr: string,
	message?: string,
...
export function isRequiredRule(ruleStr: string): boolean {
	...
	return (
		...
		parts.includes("filled") ||
		parts.includes("present")
	);
}
