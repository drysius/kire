...
declare global {
    ...
    // eslint-disable-next-line no-var
    var TriggerServerEvent: ((eventName: string, payload: any) => void) | undefined;
    // eslint-disable-next-line no-var
    var onNet: ((eventName: string, handler: (...args: any[]) => void) => void) | undefined;
    // eslint-disable-next-line no-var
    ...
    // eslint-disable-next-line no-var
    var on: ((eventName: string, handler: (...args: any[]) => void) => void) | undefined;
    // eslint-disable-next-line no-var
    var __KIREWIRE_FIVEM_CONFIG__: FiveMBridgeConfig | undefined;
}

const runtimeConfig = (globalThis as any).__KIREWIRE_FIVEM_CONFIG__ as FiveMBridgeConfig | undefined;
...
const config = {
    ...
    requestTimeoutMs: Math.max(1_000, Number(runtimeConfig?.requestTimeoutMs || 15_000)),
};
...
function emitServer(envelope: Envelope) {
    const sender = (globalThis as any).emitNet || (globalThis as any).TriggerServerEvent;
...
function registerNuiBridge() {
    ...
    if (typeof registerNuiCallbackType !== "function" || typeof onEvent !== "function") return;

    registerNuiCallbackType(config.nuiCallback);
    onEvent(`__cfx_nui:${config.nuiCallback}`, (input: any, cb: (response: any) => void) => {
