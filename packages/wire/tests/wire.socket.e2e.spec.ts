...
test.describe("Wire Socket E2E (Playwright)", () => {
    test.beforeAll(async () => {
        ...
        if (!serverProcess || serverProcess.exitCode !== null) {
            throw new Error("Failed to start docs server in socket mode. Port 3000 may already be in use.");
        }
    ...
    test("uses websocket for wire calls (no POST /_wire and no EventSource)", async ({ page }) => {
        ...
        const runtimeInfo = await page.evaluate(() => {
            ...
            return {
                transportMeta: document.querySelector('meta[name="kirewire:transport"]')?.getAttribute("content") || null,
                hasSocketCtor: typeof socketCtor,
                usesSocketAdapter: Boolean(socketCtor && wire?.adapter instanceof socketCtor),
            };
        });
