...
describe("Process Core Logic", () => {
    ...
    test("resolveComponentName should extract name from payload", () => {
        ...
        expect(resolveComponentName({ snapshot: JSON.stringify({ memo: { name: "snap-test" } }) } as any)).toBe("snap-test");
    ...
    test("applyUpdates should update properties and clear errors", async () => {
        ...
        await applyUpdates(comp, { title: "New", _private: "ignore", kire: "hack" });
