...
describe("FiveMAdapter", () => {
    ...
    function createHarness(options: ConstructorParameters<typeof FiveMAdapter>[0] = {}) {
        const kire = new Kire();
    ...
    test("returns error response for private methods", async () => {
        ...
        expect(String(responsePacket?.data?.error || "")).toContain('Method "_secret" is not callable.');
    ...
    test("blocks unauthorized $set property updates", async () => {
        ...
        expect(String(responsePacket?.data?.error || "")).toContain('Property "isAdmin" is not writable.');
