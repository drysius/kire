...
import { Component, locked, prop, renderless, validate, on } from "../src/decorators";
...
class FormComp extends LiveComponent {
	...
	@validate((v: unknown) => (typeof v === "string" && v.length >= 3 ? null : "too short"))
	@prop title = "";
...
describe("decorators + features", () => {
	test("@Component registers by name; lifecycle methods fire", async () => {
		...
		expect(snapshot.data.calls).toEqual([["mount", "booted"], { s: "arr" }] as never);
	});
	...
	test("updated<Prop> hook fires after a write", async () => {
		const w = wire();
		const { snapshot } = await w.mount("form");
		const res = await w.update({ snapshot, updates: { name: "bob" }, calls: [] });
	...
	test("@validate records an error effect for bad input", async () => {
		const w = wire();
		const { snapshot } = await w.mount("form");
		const res = await w.update({ snapshot, updates: { title: "ab" }, calls: [] });
		if ("skip" in res) throw new Error("skip");
		expect(res.effects.errors).toEqual({ title: "too short" });

		const ok = await w.update({ snapshot: res.snapshot, updates: { title: "abcd" }, calls: [] });
	...
	test("@renderless skips re-render", async () => {
		const w = wire();
		const { snapshot } = await w.mount("form");
		const res = await w.update({ snapshot, updates: {}, calls: [{ method: "ping", params: [] }] });
		if ("skip" in res) throw new Error("skip");
