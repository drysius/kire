...
describe("wire morph utilities", () => {
    ...
    test("resolveMorphKey derives stable keys for interactive wire elements", () => {
        ...
        expect(resolveMorphKey(button)).toBe("wire:click:edit(14)|target:|tag:button");
    ...
    test("morphDom passes stable key options to Alpine.morph", () => {
        ...
        expect(calls[0]!.options.key(to.firstElementChild)).toBe("wire:click:create|target:|tag:button");
    });
