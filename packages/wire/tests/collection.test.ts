...
describe("wire:collection", () => {
    ...
    test("client applies DOM collection patches with keyed fragments", () => {
        ...
        expect(target.querySelectorAll("[data-wire-collection-key]").length).toBe(1);
        ...
        expect(target.querySelectorAll("[data-wire-collection-key]").length).toBe(0);
    });
    ...
    test("server can skip html and emit collection effects", async () => {
        ...
        const response = await adapter.handleRequest({
            ...
            url: "/_wire",
            body: {
                batch: [{ id: "collection-1", method: "add", params: [], pageId: "page-1" }],
                pageId: "page-1",
            },
        }, "user-1", "session-1");
