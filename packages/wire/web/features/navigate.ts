...
function normalizeScriptKey(
	script: HTMLScriptElement,
	baseUrl: string = window.location.href,
): string {
	const type = String(script.getAttribute("type") || "text/javascript")
		.trim()
		.toLowerCase();
...
function collectExecutableScripts(
...
function hydrateScripts(
	sourceRoot: ParentNode = document.body,
	baseUrl: string = window.location.href,
) {
	const cache = getScriptCache();
...
function parseNavigateModifiers(anchor: HTMLAnchorElement): {
	replace: boolean;
} {
...
function shouldHandleNavigateClick(
	event: MouseEvent,
...
function extractConfigFromDocument(doc: Document) {
	...
	const parsedBusDelay =
		busDelayRaw === undefined ? undefined : Number(busDelayRaw);
...
function applyDocument(doc: Document, targetHref: string) {
	...
	document.dispatchEvent(
		new CustomEvent("kirewire:navigated", {
...
async function navigateTo(rawUrl: string, options: NavigateOptions = {}) {
	...
	startProgress();
	document.dispatchEvent(
		new CustomEvent("kirewire:navigate:start", {
			detail: {
	...
	try {
		...
		if (error?.name === "AbortError") {
			document.dispatchEvent(
				new CustomEvent("kirewire:navigate:abort", {
		...
		document.dispatchEvent(
			new CustomEvent("kirewire:navigate:error", {
				detail: {
					to: targetHref,
					reason: options.reason || "navigate",
		...
		if (navigationId === activeNavigationId) {
			...
			document.dispatchEvent(
				new CustomEvent("kirewire:navigate:finish", {
					detail: { to: targetHref, reason: options.reason || "navigate" },
				}),
			);
		}
