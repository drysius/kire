...
const RESERVED_KEYS = new Set(["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", " ", "Spacebar"]);
const GAME_CONTROL_KEYS = new Set(["arrowup", "arrowdown", "arrowleft", "arrowright", "w", "a", "s", "d", " ", "space", "spacebar"]);
const GAME_CONTROL_CODES = new Set(["arrowup", "arrowdown", "arrowleft", "arrowright", "keyw", "keya", "keys", "keyd", "space"]);
...
function parseExpression(expression: string): { channel: string; method?: string } | null {
    const raw = String(expression || "").trim();
    if (!raw) return null;

    const [rawChannel, rawMethod = ""] = raw.split("|", 2).map((part) => String(part || "").trim());
...
function resolveRootElement(el: HTMLElement): HTMLElement | null {
    ...
    try {
        const fromClosest = el.closest("[wire\\:id], [wire-id]") as HTMLElement | null;
    ...
    while (current) {
        if (current.hasAttribute("wire:id") || current.hasAttribute("wire-id")) return current;
...
function ensureSetEntry(map: Map<string, Set<CanvasBinding>>, key: string): Set<CanvasBinding> {
    let set = map.get(key);
    if (!set) {
...
function unregisterBinding(binding: CanvasBinding) {
    ...
    if (byComponent && byComponent.size === 0) bindingsByComponent.delete(binding.componentId);

    const byChannel = bindingsByChannel.get(binding.channel);
    byChannel?.delete(binding);
    if (byChannel && byChannel.size === 0) bindingsByChannel.delete(binding.channel);
...
function findComponentRoot(componentId: string): HTMLElement | null {
    try {
        const fromSelector = document.querySelector(`[wire\\:id="${componentId}"], [wire-id="${componentId}"]`) as HTMLElement | null;
        if (fromSelector) return fromSelector;
    } catch {
        // Some selector engines may reject ":" in attribute names.
    }

    const byWireId = document.querySelector(`[wire-id="${componentId}"]`) as HTMLElement | null;
...
function firstBindingInSet(set: Set<CanvasBinding> | undefined): CanvasBinding | null {
...
function pickFallbackBinding(preferredChannel?: string): CanvasBinding | null {
    if (preferredChannel) {
        const preferred = firstBindingInSet(bindingsByChannel.get(preferredChannel));
...
function resolveKeyboardBinding(event: KeyboardEvent): CanvasBinding | null {
    const directTarget = event.target instanceof HTMLCanvasElement
...
function bootstrapCanvases(scope: ParentNode = document) {
    const canvases = scope.querySelectorAll("canvas") as NodeListOf<HTMLCanvasElement>;
...
function ensureCanvasResolution(canvas: HTMLCanvasElement): { width: number; height: number; dpr: number } {
    const dpr = Math.max(1, window.devicePixelRatio || 1);
    const displayWidth = Math.max(1, Math.round(canvas.clientWidth || canvas.width || 320));
    const displayHeight = Math.max(1, Math.round(canvas.clientHeight || canvas.height || 320));
...
function drawRect(ctx: CanvasRenderingContext2D, color: string, x: number, y: number, size: number) {
    ctx.fillStyle = color;
...
function drawTank(
    ...
    if (isCurrent) {
        ...
        ctx.strokeRect(px + 1, py + 1, Math.max(1, width - 2), Math.max(1, height - 2));
    }
...
function defaultRenderer({
    ...
    const boardSize = Math.max(8, Math.round(toNumber(state.boardSize, mapRows.length || 13)));
    ...
    const players = Array.isArray(state.players) ? state.players.map((value) => toRecord(value)) : [];
    if (players.length > 0) {
        for (const playerValue of players) {
            const playerId = String(playerValue.id || "");
            const color = String(playerValue.color || "#62d26f");
            drawTank(ctx, playerValue, color, tile, offsetX, offsetY, playerId === currentPlayerId);
        }
    } else {
    ...
    const onlinePlayers = Math.max(1, Math.round(toNumber(state.onlinePlayers, players.length || 1)));
    const status = String(state.status || "Use arrows/WASD + Space");

    ctx.fillStyle = "#e9efff";
    ctx.font = '600 14px "Space Grotesk", sans-serif';
    ctx.textBaseline = "top";
    ctx.fillText(`Score ${score}   Lives ${lives}   Wave ${wave}   Players ${onlinePlayers}`, offsetX, Math.max(8, offsetY - 24));
    ...
    if (state.gameOver === true) {
        ...
        ctx.fillText("GAME OVER", offsetX + gridWidth / 2, offsetY + gridHeight / 2 - 10);
        ctx.font = '500 13px "Space Grotesk", sans-serif';
        ctx.fillStyle = "#ffffff";
        ctx.fillText("Press Reset to restart", offsetX + gridWidth / 2, offsetY + gridHeight / 2 + 18);
        ctx.textAlign = "left";
...
function canvasPointFromPointer(event: PointerEvent, canvas: HTMLCanvasElement): Point {
...
function emitKeyboardInput(binding: CanvasBinding, type: "keydown" | "keyup", event: KeyboardEvent) {
...
function shouldSkipRepeatedKeydown(binding: CanvasBinding, event: KeyboardEvent): boolean {
...
function ensureGlobalKeyboardHandlers() {
    ...
    const onWindowKeyDown = (event: KeyboardEvent) => {
        ...
        if (event.target instanceof HTMLCanvasElement && bindingsByCanvas.has(event.target)) return;
...
function attachInputHandlers(binding: CanvasBinding) {
    ...
    const makeCommon = (type: CanvasInputType) => (event: KeyboardEvent | PointerEvent): Omit<WireCanvasInput, "type" | "channel" | "componentId" | "method" | "at"> => {
        const maybePointer = event as PointerEvent;
        const maybeKeyboard = event as KeyboardEvent;
        const rect = canvas.getBoundingClientRect();
        const point = type === "pointer" ? canvasPointFromPointer(maybePointer, canvas) : null;
    ...
    binding.disposeFns.push(() => canvas.removeEventListener("pointerdown", onPointerDown));
    ...
    binding.disposeFns.push(() => canvas.removeEventListener("keydown", onKeyDown));
    binding.disposeFns.push(() => canvas.removeEventListener("keyup", onKeyUp));
}

function bindCanvas(el: HTMLCanvasElement, channel: string, method: string | undefined, componentId: string, state: Record<string, unknown>) {
