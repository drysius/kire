...
function detectPreviewKind(mime: string, name: string): PreviewKind {
	...
	if (/\.(png|jpe?g|gif|webp|svg|bmp|ico)$/.test(normalizedName))
		return "image";
...
function toPreviewItem(
	...
	if (typeof value === "object") {
		...
		else if (typeof raw.content === "string" && looksLikeUrl(raw.content))
			src = raw.content;
...
function renderPreview(items: PreviewItem[]): string {
	...
	for (let i = 0; i < items.length; i++) {
		...
		html.push(
			'<article class="rounded-xl border border-base-300/70 bg-base-100/80 p-2">',
		);
		if (item.kind === "image" && safeSrc) {
			html.push(
				`<img src="${safeSrc}" alt="${safeName}" class="h-auto max-h-40 w-full rounded-lg object-contain bg-base-200/40" />`,
			);
		} else if (item.kind === "audio" && safeSrc) {
			html.push('<div class="w-full min-w-56">');
			html.push(
				`<audio controls preload="none" src="${safeSrc}" class="w-full"></audio>`,
			);
			html.push("</div>");
		} else if (item.kind === "video" && safeSrc) {
			html.push(
				`<video controls preload="metadata" src="${safeSrc}" class="max-h-48 w-full rounded-lg bg-base-200/40"></video>`,
			);
		} else if (safeSrc) {
			html.push(
				`<a href="${safeSrc}" target="_blank" rel="noopener noreferrer" class="link link-primary text-sm break-all">${safeName}</a>`,
			);
		}

		html.push(
			'<div class="mt-1 flex items-center justify-between gap-2 text-xs text-base-content/60">',
		);
...
Kirewire.directive(
	"file",
	({ el, expression, modifiers, componentId, wire, cleanup }) => {
		if (!modifiers.includes("preview")) return;
		if (el instanceof HTMLInputElement && el.type === "file") return;

		const path = String(
			expression || el.getAttribute("wire:model") || "",
		).trim();
		if (!path) return;
		...
		const render = (incomingState?: any) => {
			...
			const proxyTarget =
				proxy?.__target && typeof proxy.__target === "object"
