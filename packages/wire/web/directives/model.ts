...
function getEventType(el: ModelElement, modifiers: string[]): string {
	if (
		(modifiers.includes("blur") || modifiers.includes("lazy")) &&
		isTextLikeModelElement(el)
	) {
		return "blur";
	}
	if (
...
function parseDebounce(modifiers: string[]): number {
	...
	const next = String(modifiers[index + 1] || "")
		.trim()
		.toLowerCase();
	if (!next) return 150;
...
Kirewire.directive(
	"model",
	({ el, expression, modifiers, cleanup, wire, componentId }) => {
