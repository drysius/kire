...
export class FiveMClientAdapter implements WireAdapter {
    ...
    constructor(options: FiveMClientAdapterOptions) {
        this.options = {
            ...
            requestTimeoutMs: Math.max(1_000, Number(options.requestTimeoutMs || 15_000)),
    ...
    public async call(componentId: string, method: string, params: any[] = []) {
        ...
        return new Promise((resolve, reject) => {
            const timeout = setTimeout(() => {
                ...
                reject(new Error(`FiveM request timed out after ${this.options.requestTimeoutMs}ms.`));
            ...
            const fallbackTimer = this.options.httpFallback
                ? setTimeout(() => {
                    ...
                    if (pending.fallbackTimer) clearTimeout(pending.fallbackTimer);
                    void this.callViaHttp(id, action, actionParams).then(resolve).catch(reject);
                }, 180)
                : undefined;

            this.pendingByRequest.set(requestId, { resolve, reject, timer: timeout, fallbackTimer });
    ...
    public reconfigure(next: Partial<FiveMClientAdapterOptions>) {
        ...
        this.options = {
            ...
            requestTimeoutMs: Math.max(1_000, Number(next.requestTimeoutMs || current.requestTimeoutMs)),
            httpFallback: next.httpFallback === undefined ? current.httpFallback : next.httpFallback === true,
        };
    ...
    private createNuiRequest = async (callbackName: string, payload: any): Promise<any> => {
        const response = await fetch(`https://${this.options.resourceName}/${callbackName}`, {
            method: "POST",
    ...
    private async callViaHttp(componentId: string, method: string, params: any[]) {
        const endpoint = new URL(this.options.url, window.location.origin).toString();
    ...
    private findComponentRoot(componentId: string): HTMLElement | null {
        ...
        for (let i = 0; i < nodes.length; i++) {
            ...
            const wireId = node.getAttribute("wire:id") || node.getAttribute("wire-id");
