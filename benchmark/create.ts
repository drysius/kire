	...
	for (const arg of argv) {
		...
		if (arg.startsWith("--engines=")) {
			...
			const values = raw
				...
				.filter(Boolean);
			options.engines =
				values.length > 0 ? Array.from(new Set(values)) : undefined;
			continue;
		}
...
function runDenoWorker(payload: BenchmarkPayload): Promise<any> {
	return new Promise((resolve, reject) => {
		...
		try {
			...
			if (result.error) {
				reject(
					new Error(`Failed to spawn Deno worker: ${result.error.message}`),
				);
				return;
			}
...
async function runRuntimeBenchmarks(
	...
	for (const scenario of scenarios) {
		...
		for (const engineName of engines) {
			...
			try {
				...
				const remainingSec = avgSec * (totalTasks - completedTasks);
				console.log(
					`FAILED: ${message} ` + `(ETA ${formatDuration(remainingSec)})`,
				);
			}
		}
	...
	if (failures.length > 0) {
		const formatted = failures
			.map(
				(failure) =>
					`${failure.scenario}/${failure.engine}: ${failure.message}`,
			)
			.join("\n");
		throw new Error(
			`Benchmark runtime "${runtime}" had failures:\n${formatted}`,
		);
	}
}

function runChildRuntime(runtime: RuntimeName, options: CliOptions) {
	const baseArgs = [
		CREATE_PATH,
		`--runtime=${runtime}`,
		"--skip-build",
	...
	const child =
		runtime === "node"
			? spawnSync("node", ["--import", "tsx", ...baseArgs], {
					stdio: "inherit",
				})
			: runtime === "bun"
