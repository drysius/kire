...
class Builder {
	...
	private async generateTypes(pkg: PackageInfo): Promise<void> {
		...
		if (pkg.name !== "kire") {
			const coreTypesPath = path.resolve(
				process.cwd(),
				"publish/core/dist/types/index.d.ts",
			);
			// @ts-expect-error
			tempConfig.compilerOptions.paths.kire = [coreTypesPath];
			// @ts-expect-error
			tempConfig.compilerOptions.paths["@kirejs/*"] = [
				`${relativeRoot}/packages/*/src/index.ts`,
			];
		} else {
			// @ts-expect-error
			tempConfig.compilerOptions.paths["@kirejs/*"] = [
	...
	private async runPrebuild(pkg: PackageInfo): Promise<void> {
		if (pkg.scripts?.prebuild) {
			...
			if (existsSync(sourceDist)) {
				...
				await cp(sourceDist, `${pkg.publishPath}/dist`, {
					recursive: true,
					force: true,
				});
			}
			...
			for (const [dep, ver] of Object.entries(content.dependencies)) {
				if ((ver as string).startsWith("workspace:")) {
					...
					if (depPkg) {
						...
						content.dependencies[dep] = (ver as string).replace(
							"workspace:",
							"",
						);
					}
