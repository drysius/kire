...
try {
    const result = await $`bun build ${entryFile} --outdir ${outDir} --target browser --format esm --minify --sourcemap=external`;
    ...
    if (existsSync(generatedFile)) {
        ...
        if (/from\s*["']node:/.test(content) || /import\(\s*["']node:/.test(content)) {
