 ...
 */
export const formatKireError = (e: any, $ctx: KireContext) => {
	const root = (typeof process !== "undefined" ? process.cwd() : "").replace(
		/\\/g,
		"/",
	); // Normalize root
	let displayPath = $ctx.$file.path
		? $ctx.$file.path.replace(/\\/g, "/")
		: "<anonymous>";
	...
	if (e.stack && e.kireGeneratedCode) {
		...
		if (match) {
			...
			if (lines[genLineNo]) {
				...
				if (sourceLineNo !== -1 && $ctx.$file.source) {
