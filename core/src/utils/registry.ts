...
export function directive(this: Kire<any>, def: DirectiveDefinition) {
	...
	this.$kire["~directives"].pattern = createFastMatcher(
		Object.keys(this.$kire["~directives"].records),
	);
...
export function element(this: Kire<any>, def: ElementDefinition) {
	...
	const names = this.$kire["~elements"].list.map((d) =>
		d.name instanceof RegExp ? d.name.source : d.name,
	);
	this.$kire["~elements"].pattern = createFastMatcher(names);
	return this;
}

export function existVar(
	this: Kire<any>,
	name: string | RegExp,
