...
function isInsideRoot(candidate: string, root: string, platform: any): boolean {
	...
	const relative = normalizePath(
		platform.relative(normalizedRoot, normalizedCandidate),
	);
	return (
		relative !== "" &&
		relative !== "." &&
...
export function resolvePath(
	...
	const root = normalizePath(
		platform.isAbsolute(config.root)
			? config.root
			: platform.resolve(config.root),
	);
	...
	for (const [name, rawTarget] of Object.entries(namespaces)) {
		...
		resolvedNamespaces[name] = normalizePath(
			platform.isAbsolute(rawTarget)
				? rawTarget
				: platform.resolve(root, rawTarget),
		);
	}
	...
	const ensureAllowed = (
		candidate: string,
		scopeRoots = allowedRoots,
	): string => {
		const absolute = normalizePath(
			platform.isAbsolute(candidate)
				? candidate
				: platform.resolve(root, candidate),
		);
