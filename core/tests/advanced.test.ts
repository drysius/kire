...
describe("Kire Advanced Features", () => {
	test("Namespaces: should resolve templates across multiple namespaces", async () => {
		...
		// Mock virtual files
		k["~store"].files[k.resolvePath("views/theme/layout.kire")] =
			"Theme Layout: @yield('content')";
		k["~store"].files[k.resolvePath("views/shared/button.kire")] =
			"<button>{{ text }}</button>";
	...
	test("Path Security: should block traversal outside allowed roots", async () => {
		const base = join(
			tmpdir(),
			`kire-boundary-${Date.now()}-${Math.random().toString(16).slice(2)}`,
		);
	...
	test("Path Security: should allow access inside namespace roots", async () => {
		const base = join(
			tmpdir(),
			`kire-namespace-${Date.now()}-${Math.random().toString(16).slice(2)}`,
		);
		const root = join(base, "views");
