...
export class TypescriptCompletionItemProvider
			...
			return items.map((item) => {
				if (item.range) {
					if (item.range instanceof vscode.Range) {
						...
						const iStart = mapper.toOriginal(
							r.inserting.start.line,
							r.inserting.start.character,
						);
						const iEnd = mapper.toOriginal(
							r.inserting.end.line,
							r.inserting.end.character,
						);
						const pStart = mapper.toOriginal(
							r.replacing.start.line,
							r.replacing.start.character,
						);
						const pEnd = mapper.toOriginal(
							r.replacing.end.line,
							r.replacing.end.character,
						);
