...
export class FeatureFormatting implements vscode.DocumentFormattingEditProvider {
    async provideDocumentFormattingEdits(
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            if (this.isLineInsideEmbeddedContent(document, i, embeddedBlocks)) continue;

            if (trimmed === "") {
                if (originalLine !== "") edits.push(this.replaceFullLine(i, originalLine, ""));
                continue;
            }
        ...
        for (const b of embeddedBlocks) {
            ...
            if (betweenEnd >= betweenStart) {
                ...
                if (!hasNewLine) {
                    edits.push(vscode.TextEdit.replace(betweenRange, docEol + b.openIndent));
                }
            }
        }
    ...
    private collectEmbeddedBlocks(document: vscode.TextDocument): EmbeddedBlock[] {
        ...
        for (let m: RegExpExecArray | null; (m = scriptRe.exec(text));) {
            ...
            const lang: EmbeddedLanguage =
                /lang\s*=\s*["']ts["']/i.test(attrs) || /type\s*=\s*["']text\/typescript["']/i.test(attrs)
                    ? "typescript"
                    : "javascript";
    ...
    private getLineType(line: string, parentDirectives: Map<string, string[]>): LineType {
    ...
    private replaceFullLine(lineNum: number, oldText: string, newText: string): vscode.TextEdit {
        return vscode.TextEdit.replace(
            new vscode.Range(
                new vscode.Position(lineNum, 0),
