	...
	implements vscode.CompletionItemProvider
{
	async provideCompletionItems(
		...
		if (stringMatch) {
			...
			if (attrMatch) {
				...
				if (
					def &&
					(def.type === "filepath" ||
						(Array.isArray(def.type) && def.type.includes("filepath")))
				) {
		...
		const isInJsAttr =
			/[:@]?[a-zA-Z0-9-.]+="[^"]*$/.test(linePrefix) ||
			/[:@]?[a-zA-Z0-9-.]+=.[^.]*$/.test(linePrefix); // Rough check
		...
		if (isInInterp || isInJsAttr) {
				...
				globals.forEach((def, key) => {
					if (key.startsWith(`${objectName}.`)) {
						...
						if (!propName.includes(".")) {
							...
							if (def.description || def.comment)
								item.documentation = def.description || def.comment;
							items.push(item);
						}
					}
				});
			...
			declaredByDirective.forEach((entry) => {
				...
				const kindLabel = entry.declarationKind
					? `${entry.declarationKind} `
					: "";
			...
			if (globals) {
				globals.forEach((val, key) => {
					if (!key.includes(".") && !key.includes(":")) {
						...
						if (val.description || val.comment)
							item.documentation = val.description || val.comment;
						items.push(item);
					}
				});
			}
			...
			const interfaceContext =
				tsProvider.getInterfaceContextForDocument(document);
			if (interfaceContext.thisType?.trim()) {
				const itItem = new vscode.CompletionItem(
					"it",
					vscode.CompletionItemKind.Variable,
				);
		...
		if (modifierMatch) {
			...
			if (attrName.startsWith("wire:")) {
				const modifiers = [
					...
					{
						name: "defer",
						doc: "Updates the property only on action (default).",
					},
					{
						name: "debounce",
						doc: "Debounces the update (e.g. .debounce.500ms).",
					},
		...
		if (char === "@" || linePrefix.endsWith("@")) {
			...
			kireStore.getState().directives.forEach((def) => {
				...
				if (activeParent) {
					...
					if (validParents?.includes(activeParent as string)) {
		...
		if (lastTagMatch) {
			...
			if (elementDef?.attributes) {
				...
				attrs.forEach(([name, rawDef]) => {
					const def =
						typeof rawDef === "string" ? { type: rawDef } : (rawDef as any);
					const item = new vscode.CompletionItem(
						name,
						vscode.CompletionItemKind.Property,
					);
	...
	private async provideFileCompletions(
		document: vscode.TextDocument,
		currentPath: string,
