...
function withViewStub<
	T extends { view: (view: string, data?: Record<string, any>) => any },
>(instance: T) {
...
describe("wire-example component renders", () => {
	const cases = [
		...
		},
		{
			...
			ctor: InfinityScroll,
			view: "components.infinity-scroll",
		},
...
describe("wire-example component behaviors", () => {
	...
	test("Chat sends message and clears input", () => {
		...
		expect(chat.messages[chat.messages.length - 1]!.text).toBe(
			"Hello everyone",
		);
