...
describe("docs index", () => {
    test("loads pages from markdown frontmatter", () => {
        ...
        expect(docsPages.every((page) => page.href.startsWith("/docs/"))).toBe(true);
    ...
    test("groups sections for sidebar navigation", () => {
        ...
        expect(docsNavGroups.some((group) => group.title === "Kire Essentials")).toBe(true);
        expect(packageNavGroups.some((group) => group.title === "Packages")).toBe(true);
    });

    test("search returns package references", () => {
        const results = searchDocs("wire");
        expect(results.length).toBeGreaterThan(0);
        expect(results.some((page) => page.href === "/docs/packages/wire")).toBe(true);
    });
