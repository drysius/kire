...
@end`,
  ...
  const highlightCodeBlock = (code, force = false) => {
    ...
    if (hljs) {
      ...
      if (resolvedLang && !code.classList.contains(`language-${resolvedLang}`)) {
  ...
  const getMonacoTheme = () => (getCurrentTheme() === "dark" ? "vs-dark" : "vs");
...
`;
  ...
  const syncMonacoTheme = () => {
    ...
    document.querySelectorAll("[data-kire-browser-playground]").forEach((root) => {
  ...
  const parseJsonInput = (value, label) => {
    try {
      ...
      const reason = error && error.message ? error.message : String(error || "Invalid JSON");
  ...
  const readPlaygroundState = (root) => {
    ...
    return {
      ...
      viewPath: viewPath instanceof HTMLInputElement ? viewPath.value : "views.playground",
  ...
  const writePlaygroundState = (root, state) => {
    ...
    if (template instanceof HTMLTextAreaElement) template.value = next.template || "";
    if (locals instanceof HTMLTextAreaElement) locals.value = next.locals || "{}";
    if (files instanceof HTMLTextAreaElement) files.value = next.files || "{}";
    if (viewPath instanceof HTMLInputElement) viewPath.value = next.viewPath || "views.playground";
    if (autorun instanceof HTMLInputElement) autorun.checked = next.autorun !== false;
  ...
  const persistPlaygroundState = (root) => {
    try {
      localStorage.setItem(playgroundStorageKey(root), JSON.stringify(readPlaygroundState(root)));
    } catch {}
  };
  ...
  const restorePlaygroundState = (root) => {
    ...
    setPlaygroundStatus(root, "Ready. Edit the template and run kire/browser in-place.", "");
  };
...
<body>${html}</body>
  ...
  const setupBrowserPlayground = (root) => {
    ...
    const inputs = root.querySelectorAll("[data-role='template'], [data-role='locals'], [data-role='files'], [data-role='view-path'], [data-role='autorun']");
    ...
    if (reset instanceof HTMLButtonElement) {
      reset.addEventListener("click", () => {
        writePlaygroundState(
          root,
          Object.assign({}, getPlaygroundPreset(root.getAttribute("data-preset") || "starter")),
        );
        persistPlaygroundState(root);
        void runBrowserPlayground(root);
      });
  ...
  const setupBrowserPlaygrounds = () => {
    const playgrounds = document.querySelectorAll("[data-kire-browser-playground]");
