...
import { HttpAdapter, SocketAdapter, wirePlugin } from "./lib/wire";
...
const WIRE_ROUTES: WireRoute[] = [
	...
	{
		path: "/kirewire/battle-tank",
		view: "pages.battle-tank",
		title: "Battle Tank",
	},
];
...
const DOCS_NON_PACKAGES = docsPages.filter(
	(item) => item.section !== "Packages",
);
...
async function bootstrap() {
	...
	console.log(
		`[docs] serving kire/browser runtime from ${KIRE_BROWSER_DIST_DIR}`,
	);
	console.log(
		`Check it out at http://localhost:3000 (${USE_SOCKET ? "socket mode" : "http+sse mode"})`,
	);
}
...
async function ensureKireBrowserRuntime() {
	...
	console.log(
		`[docs] building kire/browser runtime from ${KIRE_BROWSER_ENTRY}`,
	);
	...
	if (
		...
		throw new Error(
			"Built kire/browser bundle still references node:* modules.",
		);
	}
}
...
async function setupKire(engine: KireEngine, wire: any): Promise<WireEngine> {
	engine.plugin(KireMarkdown, { codeBlockClass: "mockup-code w-full" });
	engine.plugin(wire);
...
function registerSocketBridge(server: any, wireEngine: WireEngine) {
	...
	server.ws("/_wire/socket", {
		open(ws: SocketClient) {
			...
			const url = request
				? new URL(request.url)
				: new URL("http://localhost/_wire/socket");
...
function setKireGlobals(
	...
	const isActive = (targetPath: string) => {
		...
		return (
			payload.currentPath === targetPath ||
			payload.currentPath.startsWith(`${targetPath}/`)
		);
	};
...
function registerLegacyWireRedirects(server: any) {
	for (const [legacy, target] of Object.entries(LEGACY_WIRE_ROUTE_MAP)) {
		server.get(legacy, (context: DocsContext) =>
			redirect(context, target, 302),
		);
	}
}
...
function registerUnifiedWireHandler(server: any) {
	server.all("/_wire*", async (context: DocsContext) => {
		...
		const adapter = wireEngine?.options?.adapter;
		if (!adapter) {
			return respondWireError(
				context,
				"Wire adapter is not configured for this request.",
				500,
			);
		}
		...
		try {
			...
			if (status >= 500 && shouldRenderErrorHtml(context.request)) {
				const errorMessage =
					extractErrorMessage(response.result) ||
					`Wire request failed with status ${status}.`;
				return renderKireErrorResponse(
					context,
...
function extractErrorMessage(value: unknown): string {
	...
	if (value instanceof Error)
		return String(value.message || value.toString() || "");
