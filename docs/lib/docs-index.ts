...
function defaultRoute(relativeContentPath: string): string {
	const base = normalizePath(relativeContentPath).replace(
		/\.(md|markdown)$/i,
		"",
	);
...
function buildDocsPages(): DocsPage[] {
	...
	for (let i = 0; i < files.length; i++) {
		...
		const relativeContent = normalizePath(
		...
		const section = String(
			frontmatter.section || defaultSection(relativeContent),
		).trim();
...
function groupBySection(pages: DocsPage[]): NavGroup[] {
	...
	groups.sort(
		(a, b) =>
			sectionPriority(a.title) - sectionPriority(b.title) ||
...
export const docsByHref = new Map<string, DocsPage>(
	docsPages.map((item) => [item.href, item]),
);

export const docsNavGroups: NavGroup[] = groupBySection(
	docsPages.filter((item) => item.section !== "Packages"),
);
export const packageNavGroups: NavGroup[] = groupBySection(
	docsPages.filter((item) => item.section === "Packages"),
);

export const docsNav: NavItem[] = docsNavGroups.flatMap((group) => group.items);
export const packageNav: NavItem[] = packageNavGroups.flatMap(
	(group) => group.items,
);
