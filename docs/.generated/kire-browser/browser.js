...
`)}`;
  ...
  mapStackLine(line) {
    ...
    if (match && this.template) {
      ...
      const genLine = Number.parseInt(l, 10), genCol = Number.parseInt(c, 10);
...
function renderErrorHtml(e, kire, ctx) {
  ...
  if (template && e.stack) {
    ...
    if (match) {
      const rawLine = Number.parseInt(match[1], 10), genCol = Number.parseInt(match[2], 10);
      let sourceLine = -1;
...
function isInsideRoot(candidate, root, platform2) {
  ...
  if (normalizedCandidate === normalizedRoot)
    return true;
  const relative = normalizePath(platform2.relative(normalizedRoot, normalizedCandidate));
...
function resolvePath(filepath, config, platform2) {
  ...
  const root = normalizePath(platform2.isAbsolute(config.root) ? config.root : platform2.resolve(config.root));
  ...
  for (const [name, rawTarget] of Object.entries(namespaces)) {
    if (typeof rawTarget !== "string" || !rawTarget)
      continue;
    resolvedNamespaces[name] = normalizePath(platform2.isAbsolute(rawTarget) ? rawTarget : platform2.resolve(root, rawTarget));
  }
  const allowedRoots = [root, ...Object.values(resolvedNamespaces)];
  const ensureAllowed = (candidate, scopeRoots = allowedRoots) => {
    const absolute = normalizePath(platform2.isAbsolute(candidate) ? candidate : platform2.resolve(root, candidate));
