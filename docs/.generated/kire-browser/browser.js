...
var createVarThenRegex = (name) => {
  ...
  return new RegExp(`(?<![a-zA-Z0-9_$.])${escaped}(?![a-zA-Z0-9_$])(?!(?:\\s)*:)`);
};
...
class Compiler {
  ...
  containsAwaitKeyword(code) {
    if (!code)
      return false;
    const normalized = code.replace(JS_STRINGS_REGEX, '""').replace(JS_LINE_COMMENT_REGEX, "").replace(JS_BLOCK_COMMENT_REGEX, "");
  ...
  parseAttrCode(val) {
    ...
    if (pureMatch)
      return pureMatch[1];
    const res = val.replace(INTERPOLATION_GLOBAL_REGEX, (_, expr) => `\${$escape(${expr})}`);
    return `\`${res}\``;
  }
  compile(nodes, extraGlobals = [], _isDependency = false) {
    ...
    for (const id of this.identifiers) {
      if (RESERVED_KEYWORDS_REGEX.test(id) || localDecls.has(id) || id === localsAlias || id === "$props" || id === "$globals" || id === "$kire" || id === "$kire_response" || id === "$escape" || id === "NullProtoObj")
        continue;
    ...
    while (changed) {
      ...
      for (const [name, entries] of this.kire.$kire["~handlers"].exists_vars) {
        ...
        for (const entry of entries) {
          ...
          if (isUsed) {
            if (entry.unique && this._isDependency && (this.kire.$kire["~compile-context"]?.depth || 0) > 1) {
            ...
            entry.callback?.(this.createCompilerApi({
              type: "directive",
              name: "existVar",
              loc: { line: 0, column: 0 }
            }, {}, true));
...
${this.footer.join(`
`)}`;
    const bodyFooterNormalized = bodyFooterSource.replace(JS_STRINGS_REGEX, '""').replace(JS_LINE_COMMENT_REGEX, "").replace(JS_BLOCK_COMMENT_REGEX, "");
    ...
    if (this.identifiers.has(localsAlias) && createVarThenRegex(localsAlias).test(bodyFooterNormalized)) {
    ...
    if (Object.keys(this.dependencies).length > 0) {
      ...
      for (const path in this.dependencies) {
        ...
        const depCode = typeof compiledDependency?.meta?.code === "string" ? compiledDependency.meta.code : (() => {
        ...
        const asyncDep = compiledDependency?.meta?.async === undefined ? fallbackAsync : Boolean(compiledDependency.meta.async);
...
`).length;
      ...
      for (const m of this.mappings) {
        ...
        if (typeof this.body[targetBodyIndex] === "string" && this.body[targetBodyIndex].trim().startsWith("// kire-line:") && targetBodyIndex + 1 < this.body.length) {
          targetBodyIndex += 1;
        }
...
`;
  ...
  analyzeAst(nodes, idents, decls, visited) {
    ...
    for (const n of nodes) {
      ...
      if (n.args)
        n.args.forEach((a) => typeof a === "string" && scanIdents(a));
      ...
      if (n.type === "element") {
        ...
        for (const m of this.kire.$elementMatchers) {
          const d = m.def;
          if (typeof d.name === "string" && (d.name === n.tagName || d.name.includes("*") && n.tagName?.match(new RegExp(`^${d.name.replace("*", "(.*)")}$`)))) {
            def = d;
            break;
        ...
        if (def) {
          const aliasName = typeof n.tagName === "string" && n.tagName.startsWith("kire:") ? n.tagName.slice("kire:".length) : "";
        ...
        if (n.attributes) {
          for (const [key, val] of Object.entries(n.attributes)) {
            if (key.startsWith("@")) {
              ...
              const attrDecl = Array.isArray(def?.attributes) ? def.attributes.find((entry) => {
                if (!entry?.name)
                  return false;
                if (entry.name === key)
                  return true;
                if (typeof entry.name === "string" && entry.name.includes("*")) {
                  const pattern = new RegExp(`^${entry.name.replace("*", ".*")}$`);
                  return pattern.test(key);
                }
                return false;
              }) : undefined;
              const attrTypes = Array.isArray(attrDecl?.type) ? attrDecl.type : attrDecl?.type ? [attrDecl.type] : [];
              const shouldScanAsJs = key.startsWith(":") || attrTypes.some((type) => type === "javascript" || type === "js" || type === "any");
              if (shouldScanAsJs) {
                scanIdents(val);
                continue;
              }
      ...
      if (n.children)
        this.analyzeAst(n.children, idents, decls, visited);
      if (n.related)
        this.analyzeAst(n.related, idents, decls, visited);
    }
  }
  compileNodes(nodes) {
    for (const n of nodes) {
      switch (n.type) {
        ...
        case "interpolation":
          this.flushText();
          if (this.containsAwaitKeyword(n.content))
            this.markAsync();
...
`);
  ...
  createCompilerApi(node, _definition, _isExistVar = false) {
    ...
    const api = {
      ...
      prologue: (js) => {
        if (this.containsAwaitKeyword(js))
          this.markAsync();
...
var relateKireNames = (values = []) => values.map((value) => value.startsWith("kire:") ? value : `kire:${value}`);
var natives_default = (kire) => {
  kire.element({
    name: "style",
    raw: true,
    description: "Raw style block forwarded to the output without escaping child content.",
  ...
  kire.element({
    name: "script",
    raw: true,
    description: "Raw script block forwarded to the output without escaping child content.",
    ...
    kire.element({
      ...
      onCall: (api) => {
        ...
        proxyApi.getAttribute = (name) => {
          ...
          const attrTypes = Array.isArray(attrDef?.type) ? attrDef.type : attrDef?.type ? [attrDef.type] : [];
          if (typeof rawValue === "string" && attrTypes.includes("string") && !rawValue.includes("{{") && !(rawValue.trim().startsWith("{") && rawValue.trim().endsWith("}"))) {
  ...
  registerDirectiveElementAlias("unless", {
    ...
    attributes: [
      directiveElementAttr("cond", "javascript", "Expression that must evaluate falsy for the element body to render.")
    ],
    relatedTo: ["kire:else"]
  });
  registerDirectiveElementAlias("isset", {
    description: "Element alias for @isset that renders children when expr is defined and not null.",
    example: '<kire:isset expr="user.avatar"><img src="{{ user.avatar }}"></kire:isset>',
    attributes: [
      directiveElementAttr("expr", "javascript", "Expression checked for defined and non-null values.")
    ]
  });
  registerDirectiveElementAlias("include", {
    description: "Element alias for @include that renders another Kire view inline.",
    example: '<kire:include path="partials.card" locals="{ title: pageTitle }" />',
    void: true,
    attributes: [
      directiveElementAttr("path", "string", "View path that should be rendered inline."),
      directiveElementAttr("locals", "javascript", "Extra locals merged into the included view scope.")
    ]
  });
  registerDirectiveElementAlias("component", {
    description: "Element alias for @component that renders a dependency view and exposes nested slots.",
    example: '<kire:component path="layouts.card" locals="{ title: pageTitle }"><kire:slot name="header">Header</kire:slot>Body</kire:component>',
    attributes: [
      directiveElementAttr("path", "string", "View path that should be rendered as the target component."),
      directiveElementAttr("locals", "javascript", "Extra locals merged into the component props.")
    ]
  });
  registerDirectiveElementAlias("layout", {
    description: "Element alias for @layout that renders a layout component and captures nested sections.",
    example: '<kire:layout path="layouts.app"><kire:section name="content"><p>Hello</p></kire:section></kire:layout>',
    attributes: [
      directiveElementAttr("path", "string", "Layout view path to render."),
      directiveElementAttr("locals", "javascript", "Extra locals merged into the layout props.")
    ]
  });
  registerDirectiveElementAlias("extends", {
    description: "Element alias for @extends that mirrors layout-style component inheritance.",
    example: '<kire:extends path="layouts.app"><kire:section name="content"><p>Hello</p></kire:section></kire:extends>',
    attributes: [
      directiveElementAttr("path", "string", "Parent view path to render."),
      directiveElementAttr("locals", "javascript", "Extra locals merged into the parent component props.")
    ]
  });
  registerDirectiveElementAlias("slot", {
    description: "Element alias for @slot that captures named slot content for the parent component.",
    example: '<kire:slot name="header"><h1>Dashboard</h1></kire:slot>',
    attributes: [
      directiveElementAttr("name", "string", "Slot name that will be exposed to the parent component.")
    ]
  });
  registerDirectiveElementAlias("section", {
    description: "Element alias for @section that behaves like a named slot within a layout or extends block.",
    example: '<kire:section name="content"><p>Hello</p></kire:section>',
    attributes: [
      directiveElementAttr("name", "string", "Section name captured for the target layout.")
    ]
  });
  registerDirectiveElementAlias("yield", {
    description: "Element alias for @yield that renders a named slot and can fall back to a default expression.",
    example: `<kire:yield name="content" default="'<p>Empty</p>'" />`,
    void: true,
    attributes: [
      directiveElementAttr("name", "string", "Slot name to resolve from the current component props."),
      directiveElementAttr("default", "javascript", "Fallback expression rendered when the slot is missing.")
    ]
  });
  registerDirectiveElementAlias("define", {
    description: "Element alias for @define that captures a reusable named fragment.",
    example: '<kire:define name="hero"><h1>Hero</h1></kire:define>',
    attributes: [
      directiveElementAttr("name", "string", "Fragment name stored in the define registry.")
    ]
  });
  registerDirectiveElementAlias("defined", {
    description: "Element alias for @defined that renders a named fragment or its inline fallback children.",
    example: '<kire:defined name="hero"><h1>Fallback</h1></kire:defined>',
    attributes: [
      directiveElementAttr("name", "string", "Fragment name looked up in the define registry.")
    ]
  });
  registerDirectiveElementAlias("stack", {
    description: "Element alias for @stack that renders the accumulated contents of a named stack.",
    example: '<kire:stack name="scripts" />',
    void: true,
    attributes: [
      directiveElementAttr("name", "string", "Stack name to inject at the current output position.")
    ]
  });
  registerDirectiveElementAlias("push", {
    description: "Element alias for @push that appends the rendered body to a named stack.",
    example: '<kire:push name="scripts"><script src="/app.js"></script></kire:push>',
    attributes: [
      directiveElementAttr("name", "string", "Stack name that should receive the current element body.")
    ]
  });
  kire.element({
    name: "kire:else",
    relatedTo: ["kire:if", "kire:elseif"],
    description: "Fallback branch for a preceding <kire:if> or <kire:elseif> block.",
  ...
  kire.element({
    name: "kire:elseif",
    relatedTo: ["kire:if", "kire:elseif"],
    description: "Conditional branch evaluated after a previous <kire:if> or <kire:elseif>.",
  ...
  kire.element({
    name: "kire:if",
    description: "Conditional block element alternative to the @if directive syntax.",
  ...
  kire.element({
    name: "kire:for",
    description: "Loop element that iterates arrays or objects and exposes item aliases to its children.",
    example: '<kire:for items="items" as="item" index="i">{{ item }}</kire:for>',
    attributes: [
      {
        name: "items",
        type: "javascript",
  ...
  kire.element({
    name: "kire:empty",
    description: "Empty-state branch used together with loop-oriented Kire elements.",
  ...
  kire.element({
    name: "kire:switch",
    description: "Switch container for related <kire:case> and <kire:default> branches.",
    example: '<kire:switch value="status"><kire:case value="1">Draft</kire:case></kire:switch>',
    attributes: [
      {
        name: "value",
        type: "javascript",
  ...
  kire.element({
    name: "kire:case",
    description: "Case branch that matches the nearest parent <kire:switch> value.",
  ...
  kire.element({
    name: "kire:default",
    description: "Fallback branch rendered when no sibling <kire:case> matches.",
  ...
  kire.element({
    name: /^x-/,
    description: "Generic component element namespace. Use x-* tags to render registered Kire components and x-slot to define named slots.",
  ...
  <p>Body</p>
    ...
    onCall: (api) => {
      ...
      const hasChildren = Boolean(api.node.children && api.node.children.length > 0);
...
var component_default = (kire) => {
  kire.directive({
    ...
    children: true,
    description: "Captures a named slot block that will be exposed to the parent component render.",
    example: `@slot("header")
  <h1>Dashboard</h1>
@end`,
    onCall: (api) => {
  ...
  kire.directive({
    ...
    children: false,
    description: "Outputs a named slot from the current component, falling back to the default value when missing.",
  ...
  kire.directive({
    ...
    children: true,
    description: "Renders another Kire view as a component and exposes nested @slot blocks to it.",
...
@end`,
  ...
  kire.directive({
    ...
    signature: ["path:string", "locals:object"],
    children: true,
    description: "Alias for @component(path, locals?). Commonly used for page layouts.",
...
@end`,
  ...
  kire.directive({
    ...
    signature: ["path:string", "locals:object"],
    children: true,
    description: "Alias for @component(path, locals?). Mirrors Blade-style template inheritance naming.",
...
@end`,
  ...
  kire.directive({
    ...
    signature: ["name:string"],
    children: true,
    description: "Alias for @slot(name). Useful with @layout and @extends blocks.",
...
var import_default = (kire) => {
  kire.directive({
    ...
    signature: [`path:string`, `locals:object`],
    children: false,
    description: "Includes another Kire view inline and optionally merges additional locals for that render.",
...
var layout_default = (kire) => {
  ...
  kire.directive({
    ...
    signature: [`name:string`],
    children: true,
    description: "Captures a reusable named content fragment that can later be resolved by @defined.",
    example: `@define("hero")
  <h1>Hero</h1>
@end`,
    onCall: (api) => {
  ...
  kire.directive({
    ...
    signature: [`name:string`],
    children: `auto`,
    description: "Outputs a previously defined fragment or renders its inline fallback block when missing.",
...
@enddefined`,
    onCall: (api) => {
      const name = normalizeStackNameLiteral(api.getArgument(0) || api.getAttribute("name"));
      const nameCode = JSON.stringify(name);
      api.write(`$kire_response += "<!-- KIRE:defined(" + ${nameCode} + ") -->";`);
  ...
  kire.directive({
    ...
    signature: [`name:string`],
    children: false,
    description: "Injects the rendered contents that were pushed into a named stack.",
  ...
  kire.directive({
    ...
    signature: [`name:string`],
    children: true,
    description: "Appends the current block to a named stack that can later be rendered with @stack.",
...
var attributes_default = (kire) => {
  kire.directive({
    ...
    description: "Appends a single HTML attribute when the value is not null, undefined or false.",
  ...
  kire.directive({
    ...
    description: "Appends many HTML attributes from an object, array or string shorthand.",
  ...
  kire.directive({
    ...
    description: "Builds a `class` attribute from strings, arrays or keyed objects.",
  ...
  kire.directive({
    ...
    description: "Builds a `style` attribute from strings, arrays or keyed objects.",
...
var checks_default = (kire) => {
  kire.directive({
    ...
    description: "Renders the block only when the expression is defined and not null.",
...
@end`,
  ...
  kire.directive({
    ...
    description: "Fallback branch for empty collections or falsy values, often used alongside @for or @each.",
    example: `@empty(items)
  <p>No items</p>
@end`,
    onCall: (api) => {
...
var if_default = (kire) => {
  ...
  kire.directive({
    ...
    children: true,
    relatedTo: [],
    description: "Renders the block only when the expression evaluates to a truthy value.",
    example: `@if(user)
  <p>Hello {{ user.name }}</p>
@end`,
    onCall: (api) => {
  ...
  kire.directive({
    ...
    signature: [`cond:any`],
    children: true,
    description: "Inverse of @if. Renders the block when the expression is falsy.",
    example: `@unless(user)
...
var loop_default = (kire) => {
  kire.directive({
    ...
    children: true,
    description: "Iterates arrays or objects using `item of source` or `(item, index) in source` syntax.",
...
@end`,
  ...
  kire.directive({
    ...
    children: true,
    description: "Iterates over a collection and exposes the current item under a chosen variable name.",
...
var misc_default = (kire) => {
  kire.directive({
    ...
    children: false,
    description: "Type-only directive for tooling. Does not render output. Use @interface(Type) for local typing or @interface({ user: Type }, true) for workspace-global typing in editors.",
  ...
  kire.directive({
    name: `once`,
    children: true,
    description: "Ensures the wrapped block is rendered only once per render cycle.",
    example: `@once
  <script src="/app.js"></script>
@end`,
    onCall: (api) => {
  ...
  kire.directive({
    ...
    description: "Renders the block when the selected field has a validation error and exposes `$message` inside it.",
    example: `@error("email")
...
@end`,
  ...
  kire.directive({
    name: `csrf`,
    children: false,
    description: "Outputs a hidden CSRF token input using the global `csrf` value.",
    example: `@csrf()`,
    onCall: (api) => {
      api.write(`
  ...
  kire.directive({
    ...
    children: false,
    description: "Outputs a hidden `_method` input for HTTP verb spoofing in forms.",
    example: `@method("PUT")`,
    onCall: (api) => {
  ...
  kire.directive({
    ...
    children: false,
    description: "Declares a constant expression that becomes available to later template expressions.",
  ...
  kire.directive({
    ...
    children: false,
    description: "Declares a mutable variable expression that becomes available to later template expressions.",
...
var switch_default = (kire) => {
  kire.directive({
    ...
    signature: [`expr:any`],
    children: true,
    description: "Starts a switch block that can contain @case and @default branches.",
...
@end`,
  ...
  kire.directive({
    ...
    children: true,
    relatedTo: [`switch`, `case`, `default`],
    description: "Fallback branch used when no @case in the current @switch matches.",
    example: `@default
  <span>Unknown</span>`,
    onCall: (api) => {
      api.write(`default: {`);
...
var natives_default2 = (kire) => {
  kire.kireSchema({
    name: "kire-core",
    description: "Built-in directives and control-flow primitives shipped with the Kire runtime.",
    author: "Drysius",
    repository: "https://github.com/drysius/kire",
    version: "0.1.2"
  });
...
`) {
  ...
  checkDirective() {
    ...
    if (this.stack.length > 0) {
      for (let i = this.stack.length - 1;i >= 0; i--) {
        ...
        let shouldPop = false;
        if (def2?.closeBy) {
          const closeBy = Array.isArray(def2.closeBy) ? def2.closeBy : [def2.closeBy];
          if (closeBy.includes(rawName))
            shouldPop = true;
        }
        if (!shouldPop) {
          if (rawName === "end") {
    ...
    if (registered[rawName]) {
      ...
      if (m && m.index === 0) {
        ...
        if (candidate !== rawName && registered[candidate]) {
          const nextAfterCandidate = this.template[this.cursor + 1 + candidate.length];
          ...
          if (current) {
            ...
            if (candidateDef.relatedTo?.includes(current.name)) {
              ...
              if (currentDef?.closeBy) {
                const closeBy = Array.isArray(currentDef.closeBy) ? currentDef.closeBy : [currentDef.closeBy];
    ...
    if (!matchedName) {
      if (this.template[this.cursor + 1] === "{")
        return false;
      this.advance(rawName.length + 1);
      this.addNode({
        type: "directive",
  ...
  checkElement() {
    ...
    if (!isLetter && !this.kire.$elementsPattern.test(tagName))
      return false;
    ...
    const isRelated = lastSiblingTag ? !!def?.relatedTo?.includes(lastSiblingTag) : false;
    ...
    while (this.cursor < this.template.length) {
      ...
      if (this.template[this.cursor] === "(") {
        ...
        if (first === '"' || first === "'") {
          ...
          if (this.template[this.cursor] === first)
            this.advance(1);
        } else {
  ...
  hasExplicitDirectiveEnd(name, fromCursor) {
    ...
    if (!def?.closeBy) {
      ...
      return this.findUnescapedDirective(rest2, `end${name}`) !== -1 || this.findUnescapedDirective(rest2, "end") !== -1;
    }
    ...
    for (const token of closeBy) {
      if (this.findUnescapedDirective(rest, token) !== -1)
        return true;
    }
    return false;
  }
  ...
  findInterpolationEnd(from, close) {
    ...
    for (let i = from;i < this.template.length; i++) {
      ...
      if (!inQuote && depthParen === 0 && depthBracket === 0 && depthBrace === 0 && this.template.startsWith(close, i)) {
        return i;
      }
      if (inQuote) {
        if (escaped) {
...
var throwNoAPI = (name) => {
  return () => {
    throw new Error(`Platform API '${name}' is not available in browser. Provide it via store or use virtual files.`);
  };
};
var platform = {
...
var SOURCE_MAP_PREFIX = "//# sourceMappingURL=data:application/json;charset=utf-8;base64,";
var STACK_FRAME_REGEX = /^\s*at\s+(?:(.*?)\s+\()?(.+?):(\d+):(\d+)\)?$/;
...
function getTemplateMap(template) {
  if (!template)
    return null;
  if (template.map)
    return template.map;
...
function getTemplateSourceLines(template) {
  return typeof template?.source === "string" ? template.source.split(`
`) : [];
}
function getTemplateCodeLines(template) {
  return typeof template?.code === "string" ? template.code.split(`
`) : [];
}
function isTemplateFrame(file, templatePath) {
  return file.includes(templatePath) || file.includes("template.kire") || /anonymous|eval|AsyncFunction/.test(file);
}
function findNearestKireLineComment(template, generatedLine) {
...
function resolveTemplateFrameLocation(template, file, line, column) {
...
function resolveFirstTemplateLocation(stack, template) {
...
`);
  for (const line of lines) {
    ...
    if (!match)
      continue;
    const resolved = resolveTemplateFrameLocation(template, match[2], Number.parseInt(match[3], 10), Number.parseInt(match[4], 10));
    if (resolved)
      return resolved;
  }
  return null;
}
function previewCodeBlock(value, maxLength = 240) {
...
function renderAstValue(label, value) {
  return `<div class="ast-info"><span>${escapeForHtml(label)}</span><code>${escapeForHtml(value)}</code></div>`;
}
function renderAstNode(node, depth = 0, key = "0") {
  if (!node || typeof node !== "object")
    return "";
  const type = typeof node.type === "string" ? node.type : "unknown";
  const location = node.loc && typeof node.loc.line === "number" ? `L${node.loc.line}:C${node.loc.column || 1}` : "";
  let title = type;
  if (type === "directive" && node.name)
    title = `@${node.name}`;
  else if (type === "element" && node.tagName)
    title = `<${node.tagName}>`;
  ...
  if (Array.isArray(node.args) && node.args.length > 0) {
    info.push(renderAstValue("Args", node.args.map((value) => String(value)).join(", ")));
  }
  if (node.attributes && typeof node.attributes === "object") {
    const attrs = Object.entries(node.attributes);
    if (attrs.length > 0) {
      info.push(renderAstValue("Attributes", attrs.map(([name, value]) => `${name}="${value}"`).join(" ")));
    }
  }
  if (node.raw === true)
    info.push(renderAstValue("Raw", "true"));
  if (node.void === true)
    info.push(renderAstValue("Void", "true"));
  if (typeof node.wildcard === "string" && node.wildcard) {
    info.push(renderAstValue("Wildcard", node.wildcard));
  }
  const content = typeof node.content === "string" ? previewCodeBlock(node.content) : "";
  const contentBlock = content ? `<pre class="ast-code">${escapeForHtml(content)}</pre>` : "";
  ...
  return `<details class="ast-node" ${depth < 2 ? "open" : ""}>
		...
		<div class="ast-node-body">
			...
			${contentBlock}
			${children.length > 0 ? `<div class="ast-group"><div class="ast-group-title">Children (${children.length})</div><div class="ast-children">${children.map((child, index) => renderAstNode(child, depth + 1, `${key}.c${index}`)).join("")}</div></div>` : ""}
			${related.length > 0 ? `<div class="ast-group"><div class="ast-group-title">Related (${related.length})</div><div class="ast-children">${related.map((child, index) => renderAstNode(child, depth + 1, `${key}.r${index}`)).join("")}</div></div>` : ""}
		</div>
	</details>`;
}
function collectAstStats(nodes) {
...
function renderAstVisual(ast) {
  ...
  const badges = [
    ...
    ...Object.entries(stats.counts).map(([type, count]) => `<span class="ast-stat">${escapeForHtml(type)} ${count}</span>`)
  ];
  return `<div class="ast-overview">
		<div class="ast-stats">${badges.join("")}</div>
		<div class="ast-tree">${ast.map((node, index) => renderAstNode(node, 0, String(index))).join("")}</div>
	</div>`;
}
...
`)}`;
  ...
  mapStackLine(line) {
    ...
    const resolved = resolveTemplateFrameLocation(this.template, file, Number.parseInt(rawLine, 10), Number.parseInt(rawColumn, 10));
    if (!resolved)
      return line;
...
function renderErrorHtml(e, kire, ctx) {
  ...
  if (template && typeof e?.stack === "string") {
    ...
    if (resolvedLocation && sourceLines.length > 0) {
      ...
      snippet = sourceLines.slice(start, end).map((content, index) => {
...
`).filter((stackLine) => !stackLine.includes("new AsyncFunction")).map((stackLine) => `<div>${escapeForHtml(stackLine)}</div>`).join("");
...
function isInsideRoot(candidate, root, platform2) {
  ...
  if (normalizedCandidate === normalizedRoot)
    return true;
  const relative = normalizePath2(platform2.relative(normalizedRoot, normalizedCandidate));
  return relative !== "" && relative !== "." && !relative.startsWith("..") && !platform2.isAbsolute(relative);
}
function enforcePathBoundary(candidate, roots, platform2, originalPath) {
  for (const root of roots) {
    if (!root)
      continue;
    if (isInsideRoot(candidate, root, platform2))
      return candidate;
  }
...
function resolvePath(filepath, config, platform2) {
  if (!filepath || HTTP_URL_REGEX.test(filepath))
    return filepath;
  let path = normalizePath2(filepath);
  const ext = `.${config.extension}`;
  const namespaces = config.namespaces || {};
  const root = normalizePath2(platform2.isAbsolute(config.root) ? config.root : platform2.resolve(config.root));
  ...
  for (const [name, rawTarget] of Object.entries(namespaces)) {
    if (typeof rawTarget !== "string" || !rawTarget)
      continue;
    resolvedNamespaces[name] = normalizePath2(platform2.isAbsolute(rawTarget) ? rawTarget : platform2.resolve(root, rawTarget));
  }
  const allowedRoots = [root, ...Object.values(resolvedNamespaces)];
  const ensureAllowed = (candidate, scopeRoots = allowedRoots) => {
    const absolute = normalizePath2(platform2.isAbsolute(candidate) ? candidate : platform2.resolve(root, candidate));
...
class Kire {
  ...
  get $files() {
    ...
    return new Proxy(stored, {
      ...
      set: (target, prop, value) => {
        if (typeof prop === "string") {
          this.setVirtualFile(prop, value, target);
          return true;
        }
  ...
  constructor(options = new NullProtoObj) {
    ...
    if (!JS_IDENTIFIER_REGEX.test(localVariable) || RESERVED_KEYWORDS_REGEX.test(localVariable) || RESERVED_LOCAL_ALIASES.has(localVariable)) {
      throw new Error(`Invalid local_variable "${localVariable}". Use a valid JavaScript identifier that is not reserved by Kire.`);
    }
  ...
  getVirtualFileAliases(filepath) {
    const normalized = String(filepath || "").replace(/\\/g, "/");
    if (!normalized)
      return [];
    const aliases = new Set([normalized]);
    const platform2 = this["~store"].platform;
    const config = this["~store"].config;
    try {
      const resolved = resolvePath(normalized, config, platform2).replace(/\\/g, "/");
      aliases.add(resolved);
      if (platform2.isAbsolute(resolved)) {
        const relative = platform2.relative(config.root, resolved).replace(/\\/g, "/");
        if (relative && relative !== "." && !relative.startsWith("..") && !platform2.isAbsolute(relative)) {
  ...
  setVirtualFile(filepath, value, target = this["~store"].files) {
...
//# debugId=5B9FF5168FE3574864756E2164756E21
